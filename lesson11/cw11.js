...
for (let user of users) {
    ...
    but.innerText = 'add to favorite'
    but.onclick = function () {
        let favorit = JSON.parse(localStorage.getItem('favor')) || [];
    //         if (!favorit){
    //     favorit = [];
    //     favorit.push(user);   // можна і чере іф елс
    // }
    // else {
    //     favorit.push(user);
    // }
        favorit.push(user);
        localStorage.setItem('favor', JSON.stringify(favorit));
    }
    div.appendChild(but);
